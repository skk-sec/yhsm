...
This README is the complete executable customer instruction. No preliminary host, user, shell, working-directory, management-path or authorization reply is required. `PYTHONDONTWRITEBYTECODE=1` prevents Python bytecode cache files from modifying an original R12.1 checkout and is safe to retain for later qualified packages. After the run, provide only sanitized feedback through the separately provided private GitHub issue channel.
...
The authenticated GitHub login is recorded only as sanitized operator evidence (`GitHubAccount=<login>`). It is not a routing input: the selected target remains the binding resolved before login, and Stage-0 stops before clone if that account cannot read that exact target. Separate customer channels therefore use separate GitHub accounts and repository access; no customer account needs access to another customer channel.
...
Private access uses GitHub Device/Web authentication. A working secure OS credential backend is preferred and remains supported. On a headless host where no usable Secret Service is available, Stage-0 may use an isolated session-only GitHub CLI configuration in a verified RAM-backed temporary directory. Each per-run session configuration is removed after success, failure or interruption. A separate protected tmpfs cache reuses the verified login for eight hours from its original authentication, without extending that deadline on retries. It is bound to the OS user, github.com, verified account, selected repository and current boot. Every run verifies authentication, account and repository/issue access again. Customer readiness failures such as NO_GO do not remove the cache. No plaintext token or credential helper is left in the normal user configuration or cloned repository. Token-bearing GitHub authentication environment variables remain rejected; do not put tokens, passwords, credentials, private keys or authorization data in arguments.

To forget cached Stage-0 logins for the current OS user and stop, run `bash ./bootstrap.sh --forget-auth`. This works before DNS discovery, package installation or GitHub access; `--dry-run` previews it. It does not revoke the GitHub token, log out an OS keyring, or interrupt an already running customer session. A fresh normal bootstrap then asks for authentication again. Expiry uses boot time, including suspend, rather than the wall clock. A detached expiry worker normally deletes expired cache entries; every load independently rejects expired entries, even if that worker was terminated. Reboot removes tmpfs state. The host's swap policy still applies to tmpfs.

Authentication errors stop with the cached state preserved and an explicit recovery hint. A nonzero GitHub CLI status is not by itself reliable evidence of revocation; a temporary network error must not trigger another browser login. Parallel authentication starts are serialized; a second active start exits with a retry message instead of launching another login.
...
The reusable cache is independent of the single-use handoff described below. Customer cleanup removes only its per-run copy.
